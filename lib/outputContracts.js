...
const GOOGLE_FORENSIC_QUOTE_OMIT = Object.freeze(['page_id', 'page_number', 'sheet_name', 'row_number']);

const googleForensicProviderSchema = (schema) => {
  const next = JSON.parse(JSON.stringify(schema));
  const quote = next?.properties?.items?.items?.properties?.evidence_quotes?.items;
  if (!quote?.properties) return next;
  // Gemini 400s additionalProperties:false when any listed property is optional.
  // Locators are stamped later, so omit them. Require the remaining quote keys
  // (including both chunk_id and derived_evidence_id); the worker exclusive-strips.
  for (const key of GOOGLE_FORENSIC_QUOTE_OMIT) delete quote.properties[key];
  quote.required = Object.keys(quote.properties);
  return next;
};
...
export function structuredOutputForPacket(packet) {
  ...
  let schema = schemaForProvider(contract.schema, packet.provider);
  if (packet.provider === 'google' && isForensicAuditContract(packet.output_contract)) {
    schema = googleForensicProviderSchema(schema);
  }
  return { name: packet.output_contract, schema };
}
