...
import { providerCredentialConfigured } from './modelRoutingPolicy.js';
...
const providerErrorCode = async response => {
  try {
    ...
    return safeDiagnosticString(payload?.error?.status)
      || safeDiagnosticString(payload?.error?.type)
      || safeDiagnosticString(payload?.status)
      || safeDiagnosticString(typeof code === 'string' ? code : undefined);
  } catch {
    return undefined;
  }
};

const chatCompletionsBody = (packet, structuredOutput) => ({
  model: packet.model,
  messages: [
  ...
  } : packet.provider === 'meta' ? {} : { response_format: { type: 'json_object' } }),
});

const googleThinkingLevel = effort => {
  if (effort === 'low' || effort === 'medium' || effort === 'high') return effort;
  return undefined;
};

const googleCandidateText = payload => (payload?.candidates?.[0]?.content?.parts || [])
...
export async function invokeProvider(packet, { fetchFn = fetch, env = process.env, timeoutMs = 540_000, signal } = {}) {
  ...
  try {
    ...
    if (packet.provider === 'openai') {
      ...
      headers = { 'Content-Type': 'application/json', Authorization: `Bearer ${key}` };
      body = chatCompletionsBody(packet, structuredOutput);
    } else if (packet.provider === 'meta') {
      // Official Model API docs and the working sibling invocation use
      // MODEL_API_KEY. META_API_KEY remains an alias only.
      const key = env.MODEL_API_KEY || env.META_API_KEY;
      if (!key) throw new GovernanceError('PROVIDER_NOT_CONFIGURED');
      url = 'https://api.meta.ai/v1/chat/completions';
      headers = { 'Content-Type': 'application/json', Authorization: `Bearer ${key}` };
      body = chatCompletionsBody(packet, structuredOutput);
    } else if (packet.provider === 'google') {
      const key = env.GEMINI_API_KEY || env.GOOGLE_API_KEY;
      if (!key) throw new GovernanceError('PROVIDER_NOT_CONFIGURED');
      const thinkingLevel = googleThinkingLevel(packet.settings.reasoning_effort);
      if (packet.settings.reasoning_effort && !thinkingLevel) throw new GovernanceError('PROVIDER_NOT_CONFIGURED');
      url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(packet.model)}:generateContent`;
      headers = { 'Content-Type': 'application/json', 'x-goog-api-key': key };
      body = {
        systemInstruction: packet.system_instruction
          ? { parts: [{ text: packet.system_instruction }] }
          : undefined,
        contents: [{ role: 'user', parts: packet.parts.map(p => ({ text: p.text })) }],
        generationConfig: {
          maxOutputTokens: packet.settings.max_tokens,
    ...
    if (!providerCredentialConfigured(packet.provider, env)) {
      throw new GovernanceError('PROVIDER_NOT_CONFIGURED');
    }
    ...
    if ((packet.provider === 'xai' || packet.provider === 'meta') && payload.choices?.[0]?.finish_reason === 'length') {
      throw new GovernanceError('INCOMPLETE_RESPONSE', 400, 'MAX_OUTPUT_TOKENS');
    }
    if ((packet.provider === 'xai' || packet.provider === 'meta') && payload.choices?.[0]?.message?.refusal) {
      throw new GovernanceError('INCOMPLETE_RESPONSE', 400, 'PROVIDER_REFUSAL');
    }
    if (packet.provider === 'google') {
      const finishReason = payload.candidates?.[0]?.finishReason;
      if (finishReason === 'MAX_TOKENS') {
        throw new GovernanceError('INCOMPLETE_RESPONSE', 400, 'MAX_OUTPUT_TOKENS');
      }
      if (finishReason === 'SAFETY' || finishReason === 'BLOCKLIST' || finishReason === 'PROHIBITED_CONTENT' || finishReason === 'SPII') {
        throw new GovernanceError('INCOMPLETE_RESPONSE', 400, 'PROVIDER_REFUSAL');
      }
      ...
      if (!payload.candidates?.length) {
        throw new GovernanceError('INCOMPLETE_RESPONSE', 400, 'PROVIDER_INCOMPLETE');
      }
    }
    ...
    if (packet.provider === 'xai' || packet.provider === 'meta') {
      return {
    ...
    if (packet.provider === 'google') {
      return {
        text: googleCandidateText(payload),
        usage: googleUsage(payload.usageMetadata),
      };
    }
    return {
