...
assert.equal(coverage.total_object_count,80);
assert.equal(coverage.objects.length,80);
...
assert.equal(coverage.unsupported_count,55);
assert.equal(coverage.analyzer_available_count+coverage.unsupported_count,80);
...
assert.equal(new Set(coverage.objects.map(object=>`${object.stream}:${object.criterion_id}`)).size,80);
...
assert.ok(coverage.objects.some(object=>object.stream==='maturity'&&object.criterion_id==='H5'));
assert.ok(coverage.objects.some(object=>object.stream==='antipattern'&&object.criterion_id==='AP-H1'));

const kbSource=await readFile(new URL('../src/knowledge_base/index.ts',import.meta.url),'utf8');const packetSource=`const expectedKnowledgeKeysFor=(batchId)=>{const domains=batchId?[batchId]:['A','B','C','D','E','F'];return domains.flatMap(domainId=>Array.from({length:5},(_,index)=>[\`maturity:\${domainId}\${index+1}\`,\`antipattern:AP-\${domainId}\${index+1}\`]).flat());};\n${kbSource.slice(kbSource.indexOf('const SHADOW_SECTION_LIMIT'),kbSource.indexOf('\nconst formatKbDoc'))}`;await writeFile(join(dir,'packetBuilder.mjs'),compile(packetSource));const {buildShadowKnowledgePacket}=await import(`file://${join(dir,'packetBuilder.mjs')}`);
