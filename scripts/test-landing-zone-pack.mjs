...
assert.equal(tactics.tactics.length, 80);
assert.equal(tactics.status, "approved_active");
assert.match(tactics.tactics[0].id, /^TAC-ORG-A1-01$/);
assert.ok(tactics.tactics.every((item) => knownIds.has(item.primary_criterion_id)), "every tactic primary must be a pack criterion");
