...
- Work 8 adapted the forensic evaluation flow on the copied kernel without waiting on Work 7 packetizer changes. After provenance, a deterministic overlay stamps quote `evidence_class` from cited chunks, caps document-only Embedded to operational and workshop-only Embedded to aspirational, converts non-platform `tested_absent` to unknown, and flags platform/document/workshop contradictions without zeroing platform facts. Scanner JSON still uses the kernel `maturity` key and three `question_results`. Models are not required to emit `evidence_class`.
...
| 2026-09-15 | 1.10 | Work 8: adapted the forensic evaluation flow. Quote class is stamped from cited chunks. Document-only Count 3 is capped to 2, workshop-only Count 3 is capped to 1, and non-platform tested absence remains unknown. Class contradictions are flagged and not voted away. Kernel batch loops, quote schema, and pair math are unchanged. |
